...
fig_sbac_prof <- data_sbac %>%
  ...
  ggplot(aes(wcpm, sbac_prof, fill = measure)) +
...
apa_table(
  ...
  align = c("l", rep("r", 6)),
  escape = FALSE,
  note = "\\emph{d} = Cohen's \\emph{d} (1988). CI = 95\\% confidence interval.",
)
```
...
apa_table(
  ...
  longtable = TRUE,
  escape = FALSE,
  note = "\\emph{h} = Cohen's \\emph{h} (1988)."
)
...
apa_table(
  list(
    ...
    `Proximal - Traditional` = pred_comp_info$data[[2]],
  ...
  align = c("l", rep("r", 6)),
  escape = FALSE,
  note = "Estimates from linear models, regressing spring comprehension on the distal or proximal CORE or Traditional CBM-R predictor for each grade; $Comprehension_i = \\beta_0 + \\beta_1CBM\\mbox{-}R_{season} + \\epsilon_i$"
) 
```
...
apa_table(
  ...
  longtable = TRUE,
  escape = FALSE,
  note = "Estimates from linear models, regressing spring SBAC ELA/L score (multiple regression) or proficiency (logistic regression) on grade level (Grade 3 or 4) and state (OR or WA); $SBAC_i = \\beta_0 + \\beta_1CBM\\mbox{-}R_{season} + Grade + State + \\epsilon_i$"
) 
```
